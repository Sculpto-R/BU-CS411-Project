...
def load_keyword_rules():
    ...
    # Opens JSON file
    f = open(
        rules_path, "r", encoding="utf-8"
    )  # "r" means open for reading, encoding = "utf-8" handles emojis, accents, etc.
    data = json.load(f)
    f.close()
    ...
    for (
        ...
        data
    ):  # data is a list of dictionaries  [{}] #loop goes through each dictionary (ruleDict)
        for key in (
            "pattern",
            "tag",
            "weight",
            "category",
        ):  # tuple of 4 keys each rule/dictionary has
...
def suggest_tags(text):
    ...
    tag_scores = (
        {}
    )  # creates empty dictionary to store each tag and its total score ex.{"rave": 1.0, "techno": 0.8}
    ...
    ):  # uses regex to see if the rule's pattern appears in the text (not case sensitive)
        tag_scores[tag] = (
            tag_scores.get(tag, 0.0) + weight
        )  # adds rule's weight to that tag's total score
